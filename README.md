...
- [`main.rs`](examples/iced-app/src/main.rs) only wires the app and re-exports
  ...
  [`window_position.rs`](examples/iced-app/src/window_position.rs), instead of
...
This is an executable v1.52 language slice, not yet a complete iced replacement.
...
native typed keyboard, pointer, pixels, padding, angles, complete default-f32
geometry primitives, and transformation values and constructors, mouse, touch, input-method,
...
splits, closed panes and list-keyed runtime pane templates, native title bars
with responsive controls, scoped maximized flags, linear backgrounds, every
concrete pane-grid state style and pane surface style, typed native runtime
style callbacks, and checked split, arrange, targeted resize, close, maximize,
