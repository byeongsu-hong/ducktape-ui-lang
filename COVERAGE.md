...
Ice 0.42 has three checked Rust boundaries:
...
| `Subscription` | partial | typed arbitrary iced `Subscription` adapters, batching, direct keyboard/mouse/window sources and system theme changes; other direct sources and combinators missing |
...
| event routing | partial | raw event subscription adapter plus native structured keyboard and mouse payloads; general native event/status types missing |
...
| mouse/touch | partial | every mouse event has a direct typed subscription; native mouse-area events and all cursor interactions are covered; touch types missing |
