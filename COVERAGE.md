...
Ice 0.90 has seven checked Rust boundaries:
...
| `Subscription` | partial | typed arbitrary iced `Subscription` adapters, batching, checked conditional activation/status filters, direct every/repeat timers, input-method/keyboard/mouse/touch/window sources and system theme changes; general `with` and output `filter_map` transforms remain indirect |
