...
Ice 1.37 has thirty-three checked Rust boundaries:
...
| explicit image allocation | missing | no first-class `image::allocate` task or typed `image::Allocation`/allocation error values yet; image handles currently allocate lazily when rendered |
