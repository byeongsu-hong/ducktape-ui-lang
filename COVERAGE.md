...
Ice 1.36 has thirty-three checked Rust boundaries:
...
| `Daemon` | native | `daemon Name` lowers to `iced::daemon`, rejects an unnamed initial window, preserves named window templates and all shared builder callbacks/settings, and standalone `exit` lowers to the native lifecycle task |
| `Animation<T>` | missing | no first-class animation state, easing/duration/repetition configuration, transition task, frame subscription, or checked interpolation projection yet |
