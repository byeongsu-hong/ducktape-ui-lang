...
pub struct Renderer {
    ...
    // Surface textures only guarantee render-attachment usage. Keep the UI in
    // a sampleable target so backdrop primitives work on every backend.
    present_target: Option<(Size<u32>, wgpu::TextureView)>,
...
impl Renderer {
    pub fn new(
        ...
        let present_blitter =
            wgpu::util::TextureBlitter::new(&engine.device, engine.format);
    ...
    fn present_target(&mut self, size: Size<u32>) -> wgpu::TextureView {
        if self
            .present_target
            .as_ref()
            .is_none_or(|(current_size, _)| *current_size != size)
        {
            let texture =
                self.engine.device.create_texture(&wgpu::TextureDescriptor {
                    label: Some("iced_wgpu sampleable present target"),
                    size: wgpu::Extent3d {
                        width: size.width,
                        height: size.height,
        ...
        self.present_target
            .as_ref()
            .expect("present target should exist")
            .1
            .clone()
    }
    ...
    pub fn present(
        ...
        let target = self.present_target(viewport.physical_size());
        let mut encoder = self.draw(clear_color, &target, viewport);

        self.present_blitter
            .copy(&self.engine.device, &mut encoder, &target, frame);
