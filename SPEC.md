# Ice Language Specification 0.51
...
marked “planned” is a design constraint, not accepted 0.51 syntax.
...
- A line whose first non-space characters are `//` is a comment. Inline and
  block comments are not part of 0.51.
...
subscription_use = subscription_source ("status=" event_status)?
                   ("when" expr)? "->" route
subscription_source
               = call
               | "every" duration
...
rich_span_property = ("size=" | "line-height=" | "line-height-px=") expr
                   ...
                   | "strike" | "strike=" expr
pane_grid      = "pane-grid" id "split=" ("horizontal" | "vertical")
                 pane_grid_property* INDENT pane_view pane_view
pane_grid_property = "ratio=" number
                   | ("width=" | "height=") length
...
`Clone` for 0.51 message payloads.
...
| `pane-grid` | two named pane trees backed by persistent split state with native click, resize and drag/drop behavior |
...
updates, while `drag` automatically applies successful drop targets:
...
This slice accepts exactly two panes and may only live in the app view, because
component/repeated instances need separately keyed persistent state. Click
routes receive the stable pane name as `str`.
...
and toggler. Font
byte loading is not part of 0.51.
...
icons, raw handles, screenshots, and callbacks remain outside 0.51.
...
extern line; 0.51 does not claim that remapping.
...
The 0.51 native backend is enough for CRUD/settings-style screens, selection,
media, hover
